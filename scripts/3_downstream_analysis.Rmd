...
library(tximport)
library(tibble)
library(rtracklayer)      # importación del GTF
library(GenomicFeatures)  # TxDb y cálculo de longitudes
...
alpha        <- 0.05
lfc_thr      <- 1
gene_col     <- "gene_id"  # ajusta al nombre de tu columna de símbolos/IDs si la tienes
...
rank_tbl <- res_A_vs_B_df %>%
  ...
  group_by(ENTREZID) %>%
  # collapse one-to-many symbol→Entrez by keeping the strongest signal
...
write.xlsx(fgsea_go_df, 
          file = "./results/2_gsea/tables/GSEA_B_vs_C.xlsx")
```
...
## 2) Build pathway–pathway edges by Jaccard overlap
gs_sel <- pathways_GO_BP[nodes$pathway]

comb_idx <- combn(seq_along(gs_sel), 2)
edge_list <- purrr::map_dfr(seq_len(ncol(comb_idx)), function(k) {
  ...
  tibble(from = names(gs_sel)[i], to = names(gs_sel)[j],
         overlap = inter, jaccard = inter / uni)
})

jacc_threshold <- 0.10
edges <- edge_list %>% filter(jaccard >= jacc_threshold)
...
edges_pruned <- dplyr::bind_rows(
  edges %>% group_by(from) %>% slice_max(jaccard, n = deg_limit, with_ties = FALSE),
  edges %>% group_by(to)   %>% slice_max(jaccard, n = deg_limit, with_ties = FALSE)
) %>%
...
# -------------------------------
# 3. Compute layout (reproducible)
# -------------------------------
set.seed(7)
layout_fr <- igraph::layout_with_fr(g, niter = 2000)
igraph::V(g)$x <- layout_fr[, 1]
igraph::V(g)$y <- layout_fr[, 2]

# -------------------------------
# 4. Plot (publication-ready)
# -------------------------------
p <- ggraph(g, layout = "manual", x = igraph::V(g)$x, y = igraph::V(g)$y) +
  ...
  geom_node_text(aes(label = term), size = 5, repel = TRUE,
                 vjust = 0.5, hjust = 0.5) +
...
ggsave(file.path(outdir, "GO_BP_network_B_vs_C.svg"), p, width = 16, height = 12)
ggsave(file.path(outdir, "GO_BP_network_B_vs_C.png"), p, width = 16, height = 12, dpi = 400)
```
...
```{r}
# save 700 x 900 in
# go_id <- "GO:0001570" # vasculogenesis - trtmnt
# go_id    <- "GO:0048514" # blood vessel morphogenesis - trtmnt
# go_id <- "GO:0034097" # response to cytokine - trtmnt
# go_id <- "GO:0006959" # humoral immune response - inf
go_id <- "GO:0006909" #phagocytosis
...
group_cols <- c("Infected"="gray60", "Uninfected"="gray75")
# --------------------------------
...
top_up   <- lfc_tbl |> slice_max(lfc, n=30, with_ties=FALSE)
top_down <- lfc_tbl |> slice_min(lfc, n=30, with_ties=FALSE)
...
group_factor <- factor(ann_ab$classification, levels=c("Infected","Uninfected"))
...
outfile_base <- "/Volumes/Carlita/carla/mirkos-lab/01-PigBrainNCC/FinalAnalysis/results/2_gsea/figures/dotplot_B_vs_C"
...
```{r}
# Crear columna de panel según el signo de NES
fgsea_go_sig <- fgsea_go_sig %>%
  mutate(
    Regulation = ifelse(NES > 0, "Upregulated", 
                        "Downregulated")
  )

# Ordenar términos por FDR dentro de cada panel
fgsea_go_sig <- fgsea_go_sig %>%
  group_by(Regulation) %>%
  mutate(term = reorder_within(term, log10FDR, Regulation)) %>%
  ungroup()

# Si no tienes tidytext, puedes definir reorder_within manualmente:
...
# Plot dividido en dos paneles sin espacios vacíos
p <- ggplot(fgsea_go_sig,
  aes(
    x    = log10FDR,
    y    = term,
