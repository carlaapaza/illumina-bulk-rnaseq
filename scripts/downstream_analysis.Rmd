...
group_ab <- factor(meta_ab$classification,
                   levels = c("A","B"),
                   labels = c("Infected treated", "Infected untreated"))
...
colores <- c("Infected treated" = "#9e90a2",
             "Infected untreated" = "#b6c2d9")
...
## 2) Build pathway–pathway edges by Jaccard overlap of member genes
...
annot <- AnnotationDbi::select(
  ...
  filter(!is.na(ENTREZID)) %>%
  distinct(SYMBOL, ENTREZID)
message("Mapped ", nrow(annot), " genes to Entrez IDs.")

vsd_df <- as.data.frame(assay(vsd)) %>%
  tibble::rownames_to_column("SYMBOL") %>%
  left_join(annot, by = "SYMBOL") %>%     # attach Entrez IDs
  filter(!is.na(ENTREZID))

vsd_long <- vsd_df %>%
  pivot_longer(cols = -c(SYMBOL, ENTREZID),
               names_to = "sample",
               values_to = "expression")
# Convert your GO list (GO_ID -> genes [Entrez IDs]) into a data frame
go_tbl <- enframe(pathways_GO_BP, name = "GO_term", value = "ENTREZID") %>%
  unnest(cols = "ENTREZID") %>%
  distinct(GO_term, ENTREZID)
ridge_df <- vsd_long %>%
  inner_join(go_tbl, by = "ENTREZID")

# Optional: limit to significantly enriched terms
ridge_df <- ridge_df %>%
  filter(GO_term %in% fgsea_go_sig$pathway)

ridge_df <- ridge_df %>%
  mutate(term = AnnotationDbi::Term(GO_term)) %>%
  mutate(term = ifelse(is.na(term) | term == "", GO_term, term))
```

```{r}
terms_use <- fgsea_go_sig %>%
  arrange(padj) %>%
  pull(pathway)

ridge_plot_df <- ridge_df %>%
  filter(GO_term %in% terms_use)

p <- ggplot(ridge_plot_df, aes(x = expression, y = term, fill = term)) +
  geom_density_ridges(alpha = 0.8, scale = 1.1, rel_min_height = 0.01, color = "#333333") +
